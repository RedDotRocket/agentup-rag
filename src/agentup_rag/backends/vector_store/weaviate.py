...
class WeaviateVectorStoreBackend(VectorStoreBackend):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        if not WEAVIATE_AVAILABLE:
            raise VectorStoreError("Weaviate is not available. Install with: pip install weaviate-client")
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.api_key:
                ...
                if auth_type == "bearer":
                    auth_config = weaviate.AuthBearerToken(access_token=self.auth_config["access_token"])
                elif auth_type == "client_credentials":
                    auth_config = weaviate.AuthClientCredentials(
                        client_secret=self.auth_config["client_secret"], scope=self.auth_config.get("scope")
                    )

            # Create Weaviate client
            self.client = weaviate.WeaviateClient(
                connection_params=weaviate.ConnectionParams.from_url(
                    url=self.url, grpc_port=443 if self.url.startswith("https") else 80
                ),
    ...
    async def _create_class_schema(self) -> None:
        ...
        try:
            ...
            if self.replication_config:
                replication_config = Configure.replication(factor=self.replication_config.get("factor", 1))

            # Create the class
            class_config = {
                "class": self.class_name,
                "description": "RAG document class for AgentUp",
    ...
    def _convert_data_type(self, data_type: str | list[str]) -> DataType:  # type: ignore
    ...
    async def add_vectors(self, vectors: list[VectorDocument], collection: str | None = None) -> None:
        ...
        try:
            ...
            for doc in vectors:
                ...
                objects.append({"uuid": self._generate_uuid(doc.id), "properties": properties, "vector": doc.embedding})
            ...
            for i in range(0, len(objects), batch_size):
                ...
                with weaviate_collection.batch.dynamic() as batch_client:
                    for obj in batch:
                        batch_client.add_object(properties=obj["properties"], uuid=obj["uuid"], vector=obj["vector"])
    ...
    async def _ensure_tenant_exists(self, tenant: str) -> None:
        ...
        try:
            ...
            if tenant not in existing_tenants:
                await self._run_async(collection.tenants.create, [{"name": tenant}])
    ...
    async def search(
        ...
        try:
            ...
            query_builder = weaviate_collection.query.near_vector(
                near_vector=query_vector, limit=k, return_metadata=MetadataQuery(score=True, explain_score=True)
            )
    ...
    def _build_weaviate_filter(self, filters: dict[str, Any]) -> _Filters | None:  # type: ignore
    ...
    async def get_collection_stats(self, collection: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            # Get object count
            count_result = await self._run_async(weaviate_collection.aggregate.over_all, total_count=True)
            ...
            return {"name": collection or "default", "exists": False, "count": 0, "error": str(e)}
    ...
    async def create_collection(self, name: str, dimension: int, description: str | None = None) -> None:
        ...
        try:
            if self.multi_tenancy_enabled:
                ...
                await self._run_async(collection.tenants.create, [tenant_config])
