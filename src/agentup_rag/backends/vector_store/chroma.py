...
class ChromaVectorStoreBackend(VectorStoreBackend):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        if not CHROMA_AVAILABLE:
            raise VectorStoreError("Chroma is not available. Install with: pip install chromadb")
        ...
        try:
            if self.host:
                ...
                if self.persist_directory:
                    settings = Settings(persist_directory=self.persist_directory, is_persistent=True)
    ...
    async def add_vectors(self, vectors: list[VectorDocument], collection: str | None = None) -> None:
        ...
        try:
            ...
            chroma_collection = await self._get_or_create_collection(collection_name, len(vectors[0].embedding))
    ...
    async def delete_vectors(self, vector_ids: list[str], collection: str | None = None) -> None:
        ...
        try:
            ...
            # Delete vectors
            await self._run_sync(chroma_collection.delete, ids=vector_ids)
    ...
    async def get_collection_stats(self, collection: str | None = None) -> dict[str, Any]:
        ...
        try:
            chroma_collection = await self._get_collection(collection_name)
            if not chroma_collection:
                return {"name": collection_name, "exists": False, "count": 0, "dimension": None}
    ...
    async def create_collection(self, name: str, dimension: int, description: str | None = None) -> None:
        ...
        try:
            # Prepare metadata
            metadata = {"dimension": dimension, "distance_function": self.distance_function}
            if description:
                metadata["description"] = description

            # Create collection
            collection = await self._run_sync(self.client.create_collection, name=name, metadata=metadata)
    ...
    async def _get_collection(self, name: str) -> ChromaCollection | None:  # type: ignore
    ...
    async def _get_or_create_collection(self, name: str, dimension: int) -> ChromaCollection:  # type: ignore
