...
class MemoryVectorStoreBackend(VectorStoreBackend):
            ...
            for vector in vectors:
                if len(vector.embedding) != expected_dim:
                    raise VectorStoreError(
                        f"Vector dimension {len(vector.embedding)} doesn't match "
    ...
    async def search(
        ...
        for i, idx in enumerate(indices[0]):
            ...
            # Find document by index (this is inefficient but works for memory store)
            # In a real implementation, we'd maintain an index-to-id mapping
            doc_list = list(coll["documents"].values())
            if idx < len(doc_list):
