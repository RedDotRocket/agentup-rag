...
class PineconeVectorStoreBackend(VectorStoreBackend):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not PINECONE_AVAILABLE:
            raise VectorStoreError(
                "Pinecone is not available. Install with: pip install pinecone-client"
            )
    ...
    async def list_collections(self) -> List[str]:
        ...
        try:
            ...
            if (hasattr(stats, 'total_vector_count') and 
                stats.total_vector_count > 0 and 
                '' not in namespaces):
                namespaces.append('')
    ...
    async def create_collection(
        ...
        if dimension != self.dimension:
            raise VectorStoreError(
                f"Collection dimension {dimension} must match index dimension {self.dimension}"
            )
